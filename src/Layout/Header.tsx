...
const Header = () => {
  ...
  return (
    <div className="fixed top-0 w-full z-50">
      <header className={`  ${isScrolled ? "bg-white/55 backdrop-blur-md" : "md:pt-6"} `}>
        <div className="container container-p py-4 flex justify-between items-center  ">
          ...
          <nav className="hidden lg:flex gap-6 items-center">
            {HeaderItems?.map((item, index) => (
              <div key={index} className="relative group cursor-pointer">
                <NavLink to={item?.link || ""} className={`flex gap-1 items-center text-lg font-medium text-gray-800 hover:text-primary transition  px-3 py-2 rounded-t-lg  ${item?.child ? "group-hover:bg-white hover:shadow-md  " : ""} `}>
                  {item.Title}
                  ...
                  >
                    {item.child?.map((sub, subIndex) => (
                      <NavLink key={subIndex} to={sub.link}>
                        <li className="!px-4 !py-2 text-sm text-gray-700 hover:bg-gray-100 hover:text-primary transition rounded-md">{sub.Title}</li>
                      </NavLink>
                    ))}
                  </ul>
                )}
          ...
          <div className="block lg:hidden gap-1 items-center text-2xl ">
            <button onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <IoClose /> : <RiMenu3Fill />}</button>
          </div>
        </div>
      </header>
      {/* Animated Mobile Menu */}
      <div className={`lg:hidden flex flex-col bg-white gap-1 mt-2 mx-3 md:mx-5 p-4 rounded-2xl overflow-hidden transition-all duration-500 ease-in-out ${menuOpen ? "max-h-[500px] opacity-100 scale-y-100" : "max-h-0 opacity-0 scale-y-0"} origin-top`}>
        {HeaderItems?.map((item, index) => (
          <div key={index} className="relative group cursor-pointer">
            {/* Main Menu Item */}
