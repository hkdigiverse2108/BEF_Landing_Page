...
const CourseCard = ({ course }: { course: CourseType }) => {
  const { subtitle = "Talk to a counsellor", image = `${ImagePath}course/CourseCardImage.jpg`, language = "हिंGLISH", syllabus: { subjectLevel = "basic" } = {}, _id = "", onCallClick } = course;
    ...
    <NavLink to={`${ROUTES.COURSE.DETAILS.replace(":id", _id)}`} className=" relative rounded-2xl overflow-hidden cursor-pointer shadow-xl flex flex-col justify-end">
      ...
      <div className="bg-white p-4 grid gap-3">
        <div className="flex flex-nowrap items-center text-xs sm:text-sm justify-between gap-2">
          ...
          <span onClick={onCallClick} className="w-full transition-colors hover:bg-input-box-dark/60 border border-input-box-dark font-semibold text-sm p-1 px-3 rounded-sm capitalize flex items-center cursor-pointer">
