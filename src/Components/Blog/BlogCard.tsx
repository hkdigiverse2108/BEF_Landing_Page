...
const BlogCard = ({ blog }: { blog: BlogType }) => {
  return (
    <div key={blog?._id} className="bg-white shadow-md rounded-xl overflow-hidden hover:shadow-lg transition-all duration-300 h-full" data-aos="fade-up">
      {/* Blog Image */}
      ...
      {/* Blog Content */}
      <div className="p-4 sm:p-6  flex flex-col justify-between gap-6  ">
        <h3 className="text-lg sm:text-xl font-semibold text-primary  ">{blog?.title}</h3>
        <p className=" text-sm sm:text-base ">{blog?.subTitle}</p>
        <Link to={`${ROUTES.BLOG.DETAILS.replace(":id", blog?._id)}`} className="inline-block text-primary font-semibold hover:underline text-sm sm:text-base">
          READ MORE
        </Link>
      </div>
    </div>
