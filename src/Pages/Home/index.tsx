...
const Home = () => {
  const { data: featuresData, isLoading: featureLoading } = useGetApiQuery({ url: URL_KEYS.FEATURE.ALL });
  const features = featuresData?.data?.feature_data || [];

  const { data: stepsData, isLoading: stepsLoading } = useGetApiQuery({ url: URL_KEYS.HOW_IT_WORK.ALL });
  ...
  const { data: aboutData, isLoading: aboutLoading } = useGetApiQuery({ url: `${URL_KEYS.ABOUT.ALL}?type=course` });
  const aboutUs = aboutData?.data?.aboutUs;

  if (featureLoading || stepsLoading || testimonialLoading || blogLoading || faqLoading || interfaceLoading || aboutLoading) return <Loader />;
